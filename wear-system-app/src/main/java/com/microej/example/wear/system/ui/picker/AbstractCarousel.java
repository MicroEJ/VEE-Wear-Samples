...
public abstract class AbstractCarousel extends Container implements Swipeable, SwipeListener {
	 ...
	 */
	protected AbstractCarousel(boolean orientation, boolean cyclic) {
	...
	public void setCursor(PagingIndicator cursor, boolean autoHideCursor) {
		...
		PagingIndicator oldCursor = this.cursor;
		if (oldCursor != null) {
			removeChild(oldCursor);
		}
		addChild(cursor);
	...
	protected void layOutChildren(int contentWidth, int contentHeight) {
		...
		if (widgetsCount > 0) {
			SwipeEventHandler swipeEventHandler = setBoundsChildren(0, 0, contentWidth, contentHeight, widgetsCount);
			this.swipeEventHandler = swipeEventHandler;
			if (autoHide) {
				swipeEventHandler.setSwipeListener(this);
			}
