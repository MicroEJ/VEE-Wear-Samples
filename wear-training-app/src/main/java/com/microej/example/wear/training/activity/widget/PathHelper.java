...
public class PathHelper {
	...
	public static Path computeThickShapeEllipseArc(float horizontalDiameter, float verticalDiameter, float thickness,
		...
		if (arcAngle < PathHelper.MIN_ANGLE) {
			startAngle = startAngle + arcAngle;
			arcAngle = -arcAngle;
		}
		startAngle %= PathHelper.FULL_ANGLE;
		...
		return PathHelper.approximateEllipseArc(horizontalOuterRadius, verticalOuterRadius, horizontalInnerRadius,
	...
	private static Path approximateEllipseArc(float horizontalOuterRadius, float verticalOuterRadius,
		...
		PathHelper.approximateEllipseSubArc(pathBuffer, nbSections, sectionAngle, horizontalOuterRadius,
				verticalOuterRadius, startAngle, tangent, false);
		...
		if (cap == Cap.PERPENDICULAR) {
			...
			PathHelper.approximateRoundedCap(pathBuffer, (innerEndX + outerEndX) / 2, (innerEndY + outerEndY) / 2,
					capRadius, startAngle + arcAngle, startAngle + arcAngle + (float) Math.PI);
		}

		// Compute second arc (backward)
		PathHelper.approximateEllipseSubArc(pathBuffer, nbSections, sectionAngle, horizontalInnerRadius,
				verticalInnerRadius, startAngle, tangent, true);

		// Compute start cap
		if (cap == Cap.PERPENDICULAR) {
			pathBuffer.lineTo(inStartX, inStartY);
		} else {
			PathHelper.approximateRoundedCap(pathBuffer, (inStartX + outerStartX) / 2, (inStartY + outerStartY) / 2,
					capRadius, startAngle + (float) Math.PI, startAngle);
		}
	...
	private static void approximateRoundedCap(Path pathBuffer, float x, float y, float radius, float startAngle,
			float endAngle) {
		PathHelper.approximateEllipseArcFragment(pathBuffer, x, y, radius, radius, startAngle, endAngle,
				PathHelper.CAP_TAN, false);
	}
	...
	private static void approximateEllipseSubArc(Path pathBuffer, int nbSections, float sectionAngle,
		...
		for (int i = 0; i < nbSections; i++) {
			...
			PathHelper.approximateEllipseArcFragment(pathBuffer, 0, 0, horizontalRadius, verticalRadius,
					sectionStartAngle, sectionEndAngle, tangent, revert);
		}
	}
