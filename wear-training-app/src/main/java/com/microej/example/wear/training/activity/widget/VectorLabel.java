...
/**
 * A widget that displays a text using vector fonts.
 */
public class VectorLabel extends Widget {
	...
	private VectorFont getFont(Style style) {
		return style.getExtraObject(VectorLabel.FONT_STYLE, VectorFont.class,
				KernelServiceProvider.getFontService().getRegularFont());
	}

	private static int getFontSize(Style style) {
		return style.getExtraInt(VectorLabel.TEXT_SIZE_STYLE, VectorLabel.DEFAULT_TEXT_SIZE);
	}
