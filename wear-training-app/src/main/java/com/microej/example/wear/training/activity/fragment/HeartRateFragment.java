...
public class HeartRateFragment extends Canvas {
	...
	private static final String HEART_ICON_PATH = "/images/heart_icon.xml";
	private final Training training;
	// Widgets
	private final Label heartRateWidget;
	private final VectorImageWidget vectorHeartImage;
	private final VectorDurationLabel durationWidget;
	private final HeartRateZone heartRateZones;
	...
	public HeartRateFragment(Training training) {
		...
		this.heartRateWidget = new Label(
				KernelServiceProvider.getHealthService().getHeartRate() + HeartRateFragment.BPM);
		this.heartRateWidget.addClassSelector(ClassIdentifiers.HR_LABEL_WIDGET);
		...
		this.heartRateZones = new HeartRateZone(1);

		this.vectorHeartImage = new VectorImageWidget(HEART_ICON_PATH, HeartRateFragment.STARTING_SCALE_VALUE);
	...
	public static void appendStyles(CascadingStylesheet stylesheet) {
		...
		style.setFont(boldValueFont.getFont(boldValueFontSize));
	}
	...
	public void startUpdateTask() {
		...
		KernelServiceProvider.getTimer().schedule(task, HeartRateFragment.UPDATE_PERIOD,
	...
	public void startUpdateImageTask() {
		...
		this.updateImageTask = task;
		KernelServiceProvider.getTimer().schedule(task, HeartRateFragment.UPDATE_IMAGE_PERIOD,
	...
	public void stopUpdateImageTask() {
		TimerTask task = this.updateImageTask;
		if (task != null) {
			task.cancel();
	...
	private void addHeartRate(Canvas canvas, int displaySize) {
		...
		canvas.addChild(this.vectorHeartImage, heartImageX, y, imageWidth, imageHeight);
	}
	...
	private void addHeartRateWidget(Canvas canvas, int displaySize) {
		...
		canvas.addChild(this.heartRateWidget, labelX, labelY, valueWidth, valueFontHeight + HeartRateFragment.TEXT_GAP);
	...
	private void addHeartRateZones(Canvas canvas, int displaySize) {
		...
		canvas.addChild(this.heartRateZones, zonesX, zonesY, hrZonesWidth, 80);
	}
	...
	private void updateHeartRateZone() {
		...
		if (hr <= HeartRateFragment.HR_ZONE_1_THRESHOLD) {
			progress = (hr * HeartRateFragment.HUNDRED) / HeartRateFragment.HR_ZONE_1_THRESHOLD; // Zone one is between
																									// heart rate 0 and
																									// 48 inclusive
			this.heartRateZones.selectZone(HeartRateFragment.ZONE_ONE);
		} else if (hr <= HeartRateFragment.HR_ZONE_2_THRESHOLD) {
			...
			this.heartRateZones.selectZone(HeartRateFragment.ZONE_TWO);// Zone two is between heart rate 49 and 96
																		// inclusive
		} else if (hr <= HeartRateFragment.HR_ZONE_3_THRESHOLD) {
			progress = ((hr - HeartRateFragment.HR_ZONE_2_THRESHOLD) * HeartRateFragment.HUNDRED)
					/ (HeartRateFragment.HR_ZONE_3_THRESHOLD - HeartRateFragment.HR_ZONE_2_THRESHOLD);
			// Zone three is between heart rate 97-144
			this.heartRateZones.selectZone(HeartRateFragment.ZONE_THREE);
		} else if (hr <= HeartRateFragment.HR_ZONE_4_THRESHOLD) {
			progress = ((hr - HeartRateFragment.HR_ZONE_3_THRESHOLD) * HeartRateFragment.HUNDRED)
					/ (HeartRateFragment.HR_ZONE_4_THRESHOLD - HeartRateFragment.HR_ZONE_3_THRESHOLD);
			// Zone four is between heart rate 145 and 192 inclusive

			this.heartRateZones.selectZone(HeartRateFragment.ZONE_FOUR);
		} else if (hr <= HeartRateFragment.HR_ZONE_5_THRESHOLD) {
			...
			// Zone five is heart rate superior to 192
			this.heartRateZones.selectZone(HeartRateFragment.ZONE_FIVE);
		} else {
			// Heart rate is capped at 240
	...
	private void updateValues() {
		...
		HealthService healthService = KernelServiceProvider.getHealthService();
		int hr = healthService.getHeartRate();
		this.heartRateWidget.setText(hr + HeartRateFragment.BPM);
