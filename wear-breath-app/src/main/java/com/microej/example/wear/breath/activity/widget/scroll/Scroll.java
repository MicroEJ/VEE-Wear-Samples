 ...
 */
package com.microej.example.wear.breath.activity.widget.scroll;
...
public class Scroll extends Container {
	 ...
	 *            Set coordinate of the visible element.
	 * @param scrollbarMaximum
	 *            the maximum bound.
	 */
	public Scroll(boolean horizontal, int initialCoordinate, int scrollbarMaximum) {
		super(true);
		this.horizontal = horizontal;
		this.scrollbar = new Scrollbar(scrollbarMaximum);
		this.scrollbar.setHorizontal(horizontal);
		this.sbVisible = true;
		this.assistant = new ScrollAssistant();
		this.allowExcess = true;
		...
		this.scrollbar.setValue(initialCoordinate);
	...
	/**
	 * Gets the scroll top padding.
	 *
	 * @return the scroll top padding.
	 */
	...
	private int treatExcess(@Nullable Widget child, int contentWidth, int contentHeight, int childOptimalWidth,
		...
		if (this.horizontal) {
			...
			if (excess > 0) {
				this.scrollbar.setMaximum(childOptimalWidth);
				if (this.sbVisible) {
			...
			if (excess > 0) {
				this.scrollbar.setMaximum(childOptimalHeight);
				if (this.sbVisible) {
	...
	public int getCurrentIndex() {
		...
		if (this.value != 0) {
			...
			while ((tmpValue > 0) && (index < (itemSizes.length - 1))) {
				tmpValue -= itemSizes[index];
				index++;
			}
		}
		return index;
	...
	public void scrollToIndex(int index, boolean animate) {
		scrollToIndex(index, animate, SwipeEventHandler.DEFAULT_DURATION);
	}
	 ...
	 *            whether the scrolling action should be animated.
	 * @param duration
	 *            the duration of the scrolling animation if {@code animate} is {@code true}.
	 * @throws UnsupportedOperationException
	 *             if this scroll content is not {@link Scrollable}.
	 * @throws UnsupportedOperationException
	 *             if this scroll content does not provide item sizes.
	 * @see Scrollable#getItemSizes()
	 */
	public void scrollToIndex(int index, boolean animate, int duration) {
		Scrollable child = this.scrollableChild;
		if (child == null) {
			throw new UnsupportedOperationException();
		}
		...
		scrollTo(position, animate, duration);
	}
	...
	public void scrollTo(int position, boolean animate) {
		scrollTo(position, animate, SwipeEventHandler.DEFAULT_DURATION);
	}

	private void scrollTo(int position, boolean animate, int duration) {
		position = limit(position);
		scrollToNoLimit(position, animate, duration);
	}

	private void scrollToNoLimit(int position, boolean animate, long duration) {
		this.scrollbar.setValue(position);
		SwipeEventHandler swipeEventHandler = this.swipeEventHandler;
		if (swipeEventHandler != null) {
			if (animate) {
				swipeEventHandler.moveTo(position, duration);
			} else {
	...
	/**
	 * Scrolls to a position. The scrolling animation defaults to {@link SwipeEventHandler#DEFAULT_DURATION}.
	 * <p>
	 * This method does not limit the position.
	 *
	 * @param position
	 *            the x or y target (depending on the orientation).
	 * @param animate
	 *            whether the scrolling action should be animated.
	 */
	public void scrollToNoLimit(int position, boolean animate) {
		scrollToNoLimit(position, animate, SwipeEventHandler.DEFAULT_DURATION);
	}
	...
	private void updateViewport(int childCoordinate) {
		...
		if (child != null) {
			if (this.horizontal) {
				updateViewport(childCoordinate + this.topPadding, child.getY());
			} else {
				updateViewport(child.getX(), childCoordinate + this.topPadding);
			}
		}
	}
